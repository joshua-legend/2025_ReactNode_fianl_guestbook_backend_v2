...
export class TokenService {
  ...
  async generateAccessToken(user: User) {
    ...
    return this.jwtService.sign(payload, { expiresIn: '15s', secret: 'lemon' });
  }

  async generateRefreshToken(user: User) {
    const payload = { sub: user.id, username: user.name };
    const refreshToken = this.jwtService.sign(payload, { expiresIn: '30s', secret: 'lemon' });
    const expiresAt = new Date(Date.now() + parseMS('30s'));
